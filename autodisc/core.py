...
class Representation:
    ...
    def calc_distance(self, representation1, representation2, goal_space_extent=None):
        ...
        if goal_space_extent is not None:
            representation1 = representation1 - goal_space_extent[:,0]
            representation1 = representation1 / (goal_space_extent[:,1] - goal_space_extent[:,0])
            
            representation2 = representation2 - goal_space_extent[:,0]
            representation2 = representation2 / (goal_space_extent[:,1] - goal_space_extent[:,0])
...
class Explorer:
    ...
    @staticmethod
    def load_explorer(directory='./results', load_data=True, run_ids=None, load_observations=None, map_location='cpu', verbose=False):
        ...
        try:
            ...
            if map_location == 'cpu':
                if hasattr(explorer, 'goal_space_representation'):
                    if hasattr(explorer.goal_space_representation, 'model'):
                        explorer.goal_space_representation.model.use_gpu = False
                elif hasattr(explorer, 'goal_space_representations'):
                    for goal_space_representation in explorer.goal_space_representations:
                        if hasattr(goal_space_representation, 'model'):
                            goal_space_representation.model.use_gpu = False
                                
                if hasattr(explorer.system, 'automaton'):
                    if hasattr(explorer.system.automaton, 'model'):
                        explorer.system.automaton.model.is_gpu = False
                        
        except:    
            # for explorer saved with versions <= 0.8.0
