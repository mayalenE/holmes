...
def plotly_meanstd_scatter_polar(data=None, config=None, **kwargs):
    '''
    ...
    if config.plotly_format.lower() == 'webgl':
        plotly_scatter_plotter = plotly.graph_objs.Scatterglpolar
    elif config.plotly_format.lower() == 'svg':
        plotly_scatter_plotter = plotly.graph_objs.Scatterpolar
    else:
    ...
    for subplot_idx, subplot_data in enumerate(data):
        ...
        for data_idx, cur_data in enumerate(subplot_data):
            ...
            mean_trace_params = dict(
                theta=np.hstack((config.thetas, config.thetas[0])),
                r=np.hstack((mean_data, mean_data[0])),
            ...
            if config.std.style.lower() == 'shaded':
                ...
                std_trace_params = dict(
                    theta=np.hstack((config.thetas, config.thetas[0], config.thetas[0], config.thetas[::-1], config.thetas[0])),
                    r=np.hstack((mean_data + std_data, mean_data[0] + std_data[0], mean_data[0] - std_data[0], mean_data[::-1] - std_data[::-1], mean_data[0] + std_data[0])),
                    fill='toself',
                ...
                raise ValueError('Unknown config.std.style ({!r})! Options: \'shaded\''.format(config.std.type))
            ...
            for cur_elem_idx in range(n_elems):
                ...
                element_trace_params = dict(
                    theta=np.hstack((config.thetas, config.thetas[0])),
                    r=np.hstack((cur_data[cur_elem_idx, :], cur_data[cur_elem_idx, 0])),
    ...
    if n_subplots > 1:
        ...
        data_list = mean_traces[0] + elem_traces[0]
