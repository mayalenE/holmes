...
class PytorchNNRepresentation(ad.core.Representation):
    ...
    def __init__(self, config=None, **kwargs):
        ...
        self.n_dims = self.model.n_latents
            
        if self.config.preprocess_observation.type == 'function':
            self.preprocess = copy.deepcopy(self.config.preprocess_observation.function)
    ...
    def load_model (self, initialization=None):
        ...
        if initialization.type == 'random_weight':
            ...
            try:
                ...
                raise ValueError('Wrong initialization.model_init_params {!r}!'.format(initialization.model_init_params)) 
    ...
    def set_model_weights(self, config=None):
        '''Samples random weights for a pytorch module.'''
        if not hasattr(config, 'type'):
            raise ValueError('the config for setting weight {!r} must have a type attribute in either ["random", "function", "specific"]'.format(config)) 
        ...
        if config.type == 'random':
            ...
            try:
                self.model.apply(config.function)
            except:
                raise ValueError('Wrong set_weights.function {!r}!'.format(config.function)) 
        
        elif config.type == 'specific':
            try:
                saved_model = torch.load(config.load_from_model_path, map_location='cpu')
                self.model.load_state_dict(saved_model['state_dict'])
            except:
                raise ValueError('unvalid model weights {!r}!'.format(config.load_from_model_path)) 
        else:
             raise ValueError('unknown config type {!r}!'.format(config.type)) 

    def train_epoch(self, train_loader):
        return self.model.train_epoch(train_loader)
    
    def valid_epoch(self, valid_loader, save_image_in_folder=None):
        return self.model.valid_epoch(valid_loader, save_image_in_folder)
    
    
    def save_model (self, epoch_idx, output_model_filepath):
    ...
    def calc(self, observations, statistics = None):
        ...
        preprocessed_observation = self.preprocess(last_state.unsqueeze(0)).unsqueeze(0)
        with torch.no_grad():
            x = Variable(preprocessed_observation)
            representation = self.model.calc(x).squeeze(0).cpu().numpy()
        return representation


    def calc_distance(self, representation1, representation2, goal_space_extent=None):
        if self.config.distance_function is None:
            return super().calc_distance(representation1, representation2, goal_space_extent)
        else:
            return self.config.distance_function(representation1, representation2, self.config, goal_space_extent)
