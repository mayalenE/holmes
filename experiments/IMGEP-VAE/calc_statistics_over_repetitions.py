    ...
    for rep_id, rep_explorer in repetition_data.items():
        ...
        for run_data in rep_explorer.data:
            if run_data.target_goal is not None and run_data.source_policy_idx is not None:
                dist = rep_explorer.visual_representation.calc_distance(run_data.target_goal, run_data.reached_goal_per_space[run_data.target_goal_space_idx], rep_explorer.goal_library_extent[run_data.target_goal_space_idx])
...
def load_data(repetition_directories):
    ...
    for repetition_directory in sorted(repetition_directories):
        ...
        sys.path.append(repetition_directory)
        explorer = ad.explorers.GoalSpaceExplorer.load_explorer(os.path.join(repetition_directory, 'results'),
                                                                run_ids=[], map_location='cpu',
                                                                load_observations=False, verbose=False)
...
if __name__ == '__main__':
    ...
    statistics = [('error_in_goalspace_between_goal_usedpolicy', calc_error_in_goalspace_between_goal_usedpolicy),
