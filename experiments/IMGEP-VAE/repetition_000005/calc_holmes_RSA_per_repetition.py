...
def collect_holmes_representations(model, test_dataset):
    # return a dict of  N*d representations per goalspace
    data = dict()
    ...
    with torch.no_grad():
        ...
        for test_data_idx in range(test_dataset.n_images):
            ...
            all_nodes_outputs = model.network.depth_first_forward_whole_tree_preorder(x)
...
if __name__ == '__main__':
    ...
    checkpoint_filepath = "training/checkpoints/current_weight_model.pth"
    if not os.path.exists("training/checkpoints/current_weight_model.pth"):
        checkpoint_filepath = "training/checkpoints/curent_weight_model.pth" #change of version

    holmes_model = gr.dnn.BaseDNN.load_checkpoint(checkpoint_filepath, use_gpu=False)

    goalspace_names = holmes_model.network.get_node_pathes()
    n_goalspaces = len(goalspace_names)
    CKA = np.zeros((n_goalspaces, n_goalspaces))

    representations_per_goalspace = collect_holmes_representations(holmes_model, test_dataset)  # G * N x d (G=n_goalspaces, N=N_images, d=n_latents)

    for i in range(n_goalspaces):
        goal_space_i = goalspace_names[i]
        for j in range(i, n_goalspaces):
            goal_space_j = goalspace_names[j]
            CKA_ij = feature_space_linear_cka(representations_per_goalspace[goal_space_i], representations_per_goalspace[goal_space_j])
            CKA[i, j] = CKA_ij
            CKA[j, i] = CKA_ij

    statistic_foler = './statistics'
    np.save(os.path.join(statistic_foler, 'holmes_RSA.npy'), CKA)
    np.savez(os.path.join(statistic_foler, 'test_dataset_representations_per_goalspace.npz'), **representations_per_goalspace)