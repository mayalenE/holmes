...
if __name__ == '__main__':
    ...
    for i in range(n_stages):
        representations_per_stage.append(collect_holmes_representations(staged_models[i], test_dataset))# N x d (N=N_images, d=n_latents)



    representations_per_module_per_stage = dict()
    CKA_per_module = dict()
    module_names = staged_models[n_stages-1].network.get_node_pathes()
    for module_name in module_names:
        representations_per_module_per_stage[module_name] = np.empty((n_stages, test_dataset.n_images, staged_models[0].config.network.parameters.n_latents))
        representations_per_module_per_stage[module_name][:] = np.nan
        CKA = np.zeros((n_stages, n_stages))
        CKA[:] = np.nan
        for i in range(n_stages):
            if module_name in representations_per_stage[i]:
                representations_per_module_per_stage[module_name][i] = representations_per_stage[i][module_name]
                for j in range(i, n_stages):
                    CKA_ij = feature_space_linear_cka(representations_per_stage[i][module_name], representations_per_stage[j][module_name])
                    CKA[i,j] = CKA_ij
                    CKA[j,i] = CKA_ij

        CKA_per_module[module_name] = CKA

    statistic_foler = './statistics'
    np.savez(os.path.join(statistic_foler, 'temporal_holmes_RSA.npz'), **CKA_per_module)
    np.savez(os.path.join(statistic_foler, 'test_dataset_representations_per_module_per_stage.npz'), **representations_per_module_per_stage)
