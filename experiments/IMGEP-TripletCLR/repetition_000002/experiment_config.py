...
def get_model_config(model_name):
    ...
    if 'network' in model_config:
        ...
        if "ProgressiveTree" in model_name:
            ...
            model_config.node_classname = "None"
            model_config.node.network.name = "Burgess"
            model_config.node.network.parameters = {"n_channels": 1, "input_size": (256,256), "n_latents": 16, "n_conv_layers": 6, "hidden_channels": 64, "hidden_dim": 512, "encoder_conditional_type": "deterministic", "feature_layer": 2}
            model_config.node.create_connections = {}

            model_config.network.parameters = {"n_channels": 1, "input_size": (256,256), "n_latents": 16, "n_conv_layers": 6, "hidden_channels": 64, "hidden_dim": 512, "encoder_conditional_type": "deterministic", "feature_layer": 2}
        ...
        ## loss
        model_config.loss.name = "TripletCLR"
        model_config.loss.parameters = {"margin": 1.0, "distance": "cosine"}
...
def get_explorer_config():
    ...
    explorer_config.visual_representation.model.name = "TripletCLR"
    ...
    explorer_config.progressive_training.n_epochs_per_stage = 50
