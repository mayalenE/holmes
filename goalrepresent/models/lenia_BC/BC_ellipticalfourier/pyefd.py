#https://github.com/hbldh/pyefd/blob/master/pyefd.py
#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""
...
def plot_efd(coeffs, locus=(0., 0.), image=None, contour=None, n=300):
    ...
    for n in _range(coeffs.shape[0]):
        ...
        ax = plt.subplot2grid((n_rows, N_half), (n//5 // N_half, n//5 % N_half))
        ax.set_title(str(n + 1))
        if contour is not None:
            ax.plot(contour[:, 1], contour[:, 0], "c--", linewidth=1)
...
def figure_efd(coeffs, locus=(0., 0.), image=None, contour=None, n=300):
    """Plot a ``[2 x (N / 2)]`` grid of successive truncations of the series.
    .. note::
        Requires `matplotlib <http://matplotlib.org/>`_!
    :param numpy.ndarray coeffs: ``[N x 4]`` Fourier coefficient array.
    ...
    for n in range(coeffs.shape[0]):
        ...
        if n in[0, coeffs.shape[0]-1]:
            axarr[plot_idx].set_title('harmonic {}'.format(n + 1))
            if contour is not None:
                axarr[plot_idx].plot(contour[:, 1], contour[:, 0], "c--", linewidth=1)
            axarr[plot_idx].plot(yt, xt, "r", linewidth=2)
            axarr[plot_idx].axis('off')
            axarr[plot_idx].set_xticklabels([])
            axarr[plot_idx].set_yticklabels([])
            if image is not None:
                axarr[plot_idx].imshow(image, plt.cm.gray)
            plot_idx += 1
