...
from goalrepresent.datasets.image.preprocess import centroid_crop_preprocess
from goalrepresent.helper.randomhelper import set_seed
from .. import pytorchnnrepresentation


class BCPatchBVAEModel():
    ...
    def __init__(self, config=None, **kwargs):
        ...
        # preprocess
        self.preprocess_crop_ratio = 8

        if self.config.set_BC_range:
            # computed on external reference dataset of 20 000 images -> np.percentile(0.01 - 0.99)
            range = np.load(os.path.join(os.path.dirname(__file__), 'reference_dataset_patchbvae_descriptors_range.npz'))
    ...
    def preprocess(self, x):
        return centroid_crop_preprocess(x, self.preprocess_crop_ratio, keep_img_size=False)
...
#     bc_patchbvae = BCPatchBVAEModel()
#     for i in range(10):
#         x = np.random.rand(256, 256)
#         z = bc_patchbvae.calc_embedding(x)
